 ...
 /* Here's the location of the locally downloaded ts.js file. If the application is a hosted one
  ...
  let jsonModelLpcation = 'https://teachablemachine.withgoogle.com/models/EJK9cI04H/';
...
/* Final outcome of the predicted output is deleivered here */
  function predictedPhrase(error, result) {
    ...
    if (error) {
      console.error(error);
      console.log('If prediction gives an error check the py code that used to train the model');
      return;
    }

    label = result[0].label;
    //This will print all the confidence values to the log.
    console.log(result);
